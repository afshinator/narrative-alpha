...
**v1.3 Changes:**
- Section 9: LLM provider/model made runtime-configurable per call slot via `llm_config.json` on the Modal Volume; settings UI added to MVP scope
- Section 12: Settings UI moved from TBD to explicit MVP scope item
- Section 14.2C (New): SERP Search Context Reference Table injection — titles, snippets, and conditional PAA entries prepended to Call 1 entity normalization prompt as a zero-cost synonym map; graceful fallback documented
...
# Step 2: Fetch full article text via Web Unlocker
POST https://api.brightdata.com/unblocker/req?zone={zone_name}
{
  "url": "{article_url}",
  "render_js": false
}
# Strip HTML boilerplate, extract raw_text_content
...
**Purpose:** Given the consensus graph, per-source graphs, omission sets, camouflage detections, and reputation data — synthesize the complete Forensic Report JSON payload.
...
- Per-source graphs with pre-computed $O_i$ values and missing node lists (canonical-resolved)
...
**Output:** Complete Contract B JSON (Section 4). No post-processing required except label injection (done in Python via threshold rules in Section 5).
...
- Reputation Warning Banner: renders for each entry in `reputation_warnings` where `warning_triggered: true`. Displays $S_a$, validation rate, warning message.
- Outlier Signal Cards: one card per entry in `outlier_signals`. Displays claim, origin outlet, validation rate, monitoring status, evaluation window.
...
#### B. Extraction Pass — Web Unlocker API

**Target URL:** `https://api.brightdata.com/unblocker/req`

> ⚠️ **Endpoint verification required before implementation:** The spec has used both `/unblocker/req` and `/web-unlocker/req` across source documents. Confirm the canonical endpoint against the current Bright Data dashboard before the code agent writes the HTTP calls. A silent 404 here will drop every article silently.

Web Unlocker handles fingerprinting and header spoofing internally. The explicit `User-Agent` and `Accept` headers below are belt-and-suspenders for guarded corporate or state media endpoints where Web Unlocker's default profile may be insufficient. They are not redundant in that context but should not be assumed to override Web Unlocker's own spoofing layer.
...
Treat all output as raw HTML. Strip boilerplate with a lightweight parser (e.g., `trafilatura` or `newspaper3k`) before passing `raw_text_content` into the doc dict.
...
## Wishlist / Todo / for next version...
- Settings page with prefs for news sources
- ability to add/remove a src automatically (via click somewhere) a src to above mentioned settings
- On discovery & ingestion flow, front end should let user click down source article via url link
