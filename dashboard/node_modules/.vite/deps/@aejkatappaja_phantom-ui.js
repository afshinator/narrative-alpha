...
// node_modules/@aejkatappaja/phantom-ui/dist/phantom-ui.js
var T = Object.defineProperty;
var B = Object.getOwnPropertyDescriptor;
var d2 = (s, o, e, r) => {
  for (var t = r > 1 ? void 0 : r ? B(o, e) : o, i = s.length - 1, n; i >= 0; i--) (n = s[i]) && (t = (r ? n(o, e, t) : n(t)) || t);
  return r && t && T(o, e, t), t;
};
var I = /* @__PURE__ */ new Set(["IMG", "SVG", "VIDEO", "CANVAS", "IFRAME", "INPUT", "TEXTAREA", "BUTTON", "HR"]);
var A = /* @__PURE__ */ new Set(["BR", "WBR"]);
function M(s) {
  if (I.has(s.tagName)) return true;
  for (let o of s.children) if (!A.has(o.tagName)) return false;
  return true;
}
function $(s) {
  var _a4;
  for (let o of s.childNodes) if (o.nodeType === Node.TEXT_NODE && ((_a4 = o.textContent) == null ? void 0 : _a4.trim())) return true;
  return false;
}
function _(s, o) {
  let e = [];
  function r(t) {
    let i = t.getBoundingClientRect(), n = Number(t.getAttribute("data-shimmer-width")) || 0, m = Number(t.getAttribute("data-shimmer-height")) || 0, b = n > 0 || m > 0, g = n || i.width, h = m || i.height;
    if ((g === 0 || h === 0) && !b || t.hasAttribute("data-shimmer-ignore")) return;
    if (t.hasAttribute("data-shimmer-no-children") || M(t)) {
      let p = getComputedStyle(t).borderRadius;
      if ((t.tagName === "TD" || t.tagName === "TH") && $(t) && !n) {
        let f = document.createElement("span");
        f.style.visibility = "hidden", f.style.position = "absolute", f.textContent = t.textContent, t.appendChild(f);
        let E = f.getBoundingClientRect();
        t.removeChild(f), e.push({ x: i.left - o.left, y: i.top - o.top, width: Math.min(E.width, i.width), height: h, borderRadius: p === "0px" ? "" : p });
        return;
      }
      e.push({ x: i.left - o.left, y: i.top - o.top, width: g, height: h, borderRadius: p === "0px" ? "" : p });
      return;
    }
    for (let a of t.children) r(a);
  }
  return r(s), e;
}
function k(s, o) {
  ...
  let r = getComputedStyle(s), t = r.backgroundColor, i = r.borderWidth, n = r.borderStyle, m = r.borderColor, b = r.boxShadow, g = r.borderRadius, h = t === "rgba(0, 0, 0, 0)" || t === "transparent", c = n !== "none" && i !== "0px", a = b !== "none" && b !== "";
  if (h && !c && !a) return null;
  let p = c ? `${i} ${n} ${m}` : "";
  return { x: e.left - o.left, y: e.top - o.top, width: e.width, height: e.height, borderRadius: g === "0px" ? "" : g, backgroundColor: h ? "" : t, border: p, boxShadow: a ? b : "" };
}
function x(s, o) {
  let e = null, r = new ResizeObserver(() => {
    e !== null && cancelAnimationFrame(e), e = requestAnimationFrame(() => {
      e = null, o();
    });
  });
  return r.observe(s), r;
}
var w = "phantom-ui-loading-styles";
function S() {
  if (document.getElementById(w)) return;
  let s = document.createElement("style");
  s.id = w, s.textContent = `
		phantom-ui[loading] * {
...
`;
var _a3;
var l = (_a3 = class extends LitElement {
  constructor() {
    super(...arguments);
    this.loading = false;
    this.shimmerDirection = "ltr";
    this.shimmerColor = "rgba(128, 128, 128, 0.3)";
    this.backgroundColor = "rgba(128, 128, 128, 0.2)";
    this.duration = 1.5;
    this.fallbackRadius = 4;
  ...
  disconnectedCallback() {
    super.disconnectedCallback(), this._teardownObservers(), this._clearRevealTimeout();
  }
  willUpdate(e) {
    e.has("loading") && !this.loading && this.reveal > 0 && this._blocks.length > 0 && (this._revealing = true);
  }
  updated(e) {
    (e.has("count") || e.has("countGap")) && this.loading && this._scheduleMeasure(), e.has("loading") && (this.setAttribute("aria-busy", String(this.loading)), this.loading ? (this._revealing = false, this._clearRevealTimeout(), this._scheduleMeasure(), this._setupObservers()) : this._revealing ? (this._teardownObservers(), this._revealTimeout = setTimeout(() => {
      this._revealing = false, this._blocks = [], this._revealTimeout = null, this.style.minHeight = "";
    }, this.reveal * 1e3)) : (this._blocks = [], this._teardownObservers(), this.style.minHeight = ""));
  }
  render() {
    let e = styleMap({ "--shimmer-color": this.shimmerColor, "--shimmer-duration": `${this.duration}s`, "--shimmer-bg": this.backgroundColor, "--reveal-duration": `${this.reveal}s`, "--shimmer-direction": this.shimmerDirection }), r = this.loading || this._revealing;
    return html`
      <slot></slot>
      ${r ? html`
            <div
  ...
  _scheduleMeasure() {
    this._measureScheduled || (this._measureScheduled = true, requestAnimationFrame(() => {
      this._measureScheduled = false, this._measure();
    }));
  }
  _measure() {
    var _a4;
    ...
    let r = (_a4 = this.shadowRoot) == null ? void 0 : _a4.querySelector("slot");
    if (!r) return;
    let t = r.assignedElements({ flatten: true }), i = [];
    for (let n of t) {
      let m = _(n, e);
      i.push(...m);
    }
    if (this.count > 1 && i.length > 0) {
      let n = 0;
      for (let h of t) {
        let c = h.getBoundingClientRect();
        n = Math.max(n, c.bottom - e.top);
      }
      let m = [];
      for (let h of t) {
        let c = k(h, e);
      ...
      let b = [...i];
      for (let h = 1; h < this.count; h++) {
        let c = h * (n + this.countGap);
        for (let a of m) i.push({ x: a.x, y: a.y + c, width: a.width, height: a.height, borderRadius: a.borderRadius, isContainer: true, containerBg: a.backgroundColor, containerBorder: a.border, containerShadow: a.boxShadow });
        for (let a of b) i.push({ ...a, y: a.y + c });
      }
      let g = this.count * n + (this.count - 1) * this.countGap;
      this.style.minHeight = `${g}px`;
    } else this.style.minHeight = "";
    this._blocks = i, this._mutationObserver && this._mutationObserver.observe(this, { childList: true, subtree: true, attributes: true });
  }
  _setupObservers() {
    this._teardownObservers(), this._resizeObserver = x(this, () => {
      this._scheduleMeasure();
    }), this._mutationObserver = new MutationObserver(() => {
      this._scheduleMeasure();
    }), this._mutationObserver.observe(this, { childList: true, subtree: true, attributes: true }), this._loadHandler = () => this._scheduleMeasure(), this.addEventListener("load", this._loadHandler, true);
  }
  _teardownObservers() {
    this._resizeObserver && (this._resizeObserver.disconnect(), this._resizeObserver = null), this._mutationObserver && (this._mutationObserver.disconnect(), this._mutationObserver = null), this._loadHandler && (this.removeEventListener("load", this._loadHandler, true), this._loadHandler = null);
  }
  _clearRevealTimeout() {
    this._revealTimeout !== null && (clearTimeout(this._revealTimeout), this._revealTimeout = null);
  }
  _renderBlocks() {
    return this._blocks.map((e, r) => {
      let t = e.borderRadius || `${this.fallbackRadius}px`, i = { left: `${e.x}px`, top: `${e.y}px`, width: `${e.width}px`, height: `${e.height}px`, "border-radius": t };
      if (e.isContainer) {
        let m = { ...i };
        return e.containerBg && (m.background = e.containerBg), e.containerBorder && (m.border = e.containerBorder), e.containerShadow && (m["box-shadow"] = e.containerShadow), html`<div
          class="shimmer-container-block"
          style=${styleMap(m)}
        >${this.debug ? html`<span class="debug-label" data-kind="container">C${r}</span>` : nothing}</div>`;
      }
      let n = { ...i, background: `var(--shimmer-bg, ${this.backgroundColor})` };
      return this.stagger > 0 && (n["animation-delay"] = `${r * this.stagger}s`), html`<div class="shimmer-block" style=${styleMap(n)}>${this.debug ? html`<span class="debug-label">${r}</span>` : nothing}</div>`;
    });
  }
}, _a3.styles = C, _a3);
d2([property({ type: Boolean, reflect: true, converter: { fromAttribute: (e) => e !== null && e !== "false", toAttribute: (e) => e ? "" : null } })], l.prototype, "loading", 2), d2([property({ attribute: "shimmer-direction", reflect: true })], l.prototype, "shimmerDirection", 2), d2([property({ attribute: "shimmer-color" })], l.prototype, "shimmerColor", 2), d2([property({ attribute: "background-color" })], l.prototype, "backgroundColor", 2), d2([property({ type: Number })], l.prototype, "duration", 2), d2([property({ type: Number, attribute: "fallback-radius" })], l.prototype, "fallbackRadius", 2), d2([property({ reflect: true })], l.prototype, "animation", 2), d2([property({ type: Number })], l.prototype, "stagger", 2), d2([property({ type: Number })], l.prototype, "reveal", 2), d2([property({ type: Number, converter: (e) => Math.max(1, Math.round(Number(e) || 1)) })], l.prototype, "count", 2), d2([property({ type: Number, attribute: "count-gap", converter: (e) => Math.max(0, Number(e) || 0) })], l.prototype, "countGap", 2), d2([property({ type: Boolean, reflect: true })], l.prototype, "debug", 2), d2([state()], l.prototype, "_blocks", 2), d2([state()], l.prototype, "_revealing", 2);
customElements.get("phantom-ui") || customElements.define("phantom-ui", l);
export {
  l as PhantomUi
};
...
lit-html/development/directives/style-map.js:
   ...
   *)
*/
//# sourceMappingURL=@aejkatappaja_phantom-ui.js.map
