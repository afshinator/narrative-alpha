...
export function Zone3({ report }: Zone3Props) {
	...
	const triggeredWarnings = reputation_warnings.filter(
		(w) => w.warning_triggered,
	);

	return (
		<div className="zone">
			<div className="zone-header zone-header--zone3">
				<svg
					className="icon"
					viewBox="0 0 24 24"
					fill="none"
			...
			<div className="zone-body">
				{triggeredWarnings.length > 0 ? (
						...
						<div key={w.source_domain} className="rep-warning">
							...
							<div className="rep-warning-meta">
								<span className="rep-warning-meta-item">
									Scatter-shot factor:{" "}
									<strong>
										{(w.scatter_shot_anomaly_factor * 100).toFixed(0)}%
									</strong>
								</span>
								<span className="rep-warning-meta-item">
									Label:{" "}
									<span
										className={`badge badge-${w.scatter_shot_label === "HIGH" ? "high" : w.scatter_shot_label === "MED" ? "med" : "low"}`}
									>
										{w.scatter_shot_label}
									</span>
								</span>
								<span className="rep-warning-meta-item">
									Validation rate:{" "}
									<strong>
										{(w.historical_origin_validation_rate * 100).toFixed(0)}%
									</strong>
								</span>
								<span className="rep-warning-meta-item">
					...
					<div className="empty-placeholder">
						No reputation warnings flagged
					</div>
				)}

				<div className="sub-header">
					Predictive narrative watch — outlier signals
				</div>
						...
						<div className="outlier-meta">
							...
							<div className="meta-row">
								...
								<span className="meta-val">
									{(
										s.outlier_origin_provenance
											.historical_origin_validation_rate * 100
									).toFixed(0)}
									%
								</span>
							</div>
							...
							>
								...
								<span className="pending-badge">
									{s.validation_tracking.current_state} ·{" "}
									{s.validation_tracking.evaluation_window_days}-day convergence
									window active
								</span>
							</div>
						</div>
				...
				{reality_divergence_zones.length > 0 ? (
					reality_divergence_zones.map((z, i) => (
						<div key={i} className="divergence-block">
							...
							<div className="div-status-row">
								<span
									className={`status-pill status-${z.consensus_stability.toLowerCase()}`}
								>
									Consensus stability: {z.consensus_stability} (
									{z.consensus_stability_score.toFixed(2)})
								</span>
								<span
									className={`status-pill status-${z.institutional_convergence.toLowerCase()}`}
								>
									...
									<div key={narrative} className="narrative-item">
										...
										<div className="narrative-outlets">
											{(z.supporting_outlets[narrative] || []).map((domain) => (
												<span key={domain} className="outlet-tag">
													{domain}
												</span>
											))}
										</div>
					...
					<div className="empty-placeholder">None detected.</div>
				)}
					...
					reality_fractures.map((f) => (
						<div key={f.fracture_id} className="fracture-card">
							<div className="fracture-header">
								<span className="fracture-topic">
									{f.topic}
									<span className="fracture-rel-tag">{f.relationship}</span>
								</span>
								<span className="fracture-status-tag">
									{f.resolution_status}
								</span>
							</div>
								...
								<div className="claim-block">
									...
									<div className="claim-outlets">
										{f.claim_a.supporting_outlets.map((d) => (
											<span key={d} className="outlet-tag">
												{d}
											</span>
										))}
									</div>
