...
describe("Zone3", () => {
	it("applies pulsing status-unresolved class to UNRESOLVED convergence", () => {
		render(<Zone3 report={baseReport} />);
		const unresolvedPill = document.querySelector(".status-unresolved");
		expect(unresolvedPill).toBeInTheDocument();
		expect(unresolvedPill).toHaveClass("status-unresolved");
	});

	it("applies pending-badge class to PENDING outlier signals", () => {
		render(<Zone3 report={baseReport} />);
		const pendingBadge = document.querySelector(".pending-badge");
		expect(pendingBadge).toBeInTheDocument();
		expect(pendingBadge).toHaveClass("pending-badge");
	});
