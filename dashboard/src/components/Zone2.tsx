...
export function Zone2({ report }: Zone2Props) {
	...
	return (
		<div className="zone">
			<div className="zone-header zone-header--zone2">
				<svg
					className="icon"
					viewBox="0 0 24 24"
					fill="none"
			...
			<div className="zone-body">
				<div className="distortion-table-wrapper">
						...
						<tbody>
							{distortion_matrix.map((entry) => (
								<tr key={entry.source_domain}>
									...
									<td>
										<Badge
											level={entry.omission_label}
											label={`${entry.omission_index.toFixed(2)} ${entry.omission_label}`}
										/>
									</td>
									<td>
										<Badge
											level={entry.framing_volatility_label}
											label={`${entry.framing_volatility_score.toFixed(2)} ${entry.framing_volatility_label}`}
										/>
									</td>
									<td
										className="camouflage"
										aria-label={`Linguistic camouflage: ${entry.linguistic_camouflage.map((c) => `"${c.raw_expression}" → "${c.clinical_translation}"`).join("; ")}`}
									>
										{entry.linguistic_camouflage.map((cam, i) => (
											<div key={i}>
												<span className="cam-raw" aria-hidden="true">
													{cam.raw_expression}
												</span>
												...
												<span className="cam-clean" aria-hidden="true">
													{cam.clinical_translation}
												</span>
											</div>
										))}
									</td>
						...
						{narrative_regime_shifts.map((shift) => (
							<div key={shift.shift_id} className="regime-card">
								<div className="regime-shift">
									<span className="term-old">
										{shift.detected_shift.previous_term}
									</span>
									<span className="term-arrow"> → </span>
									<span className="term-new">
										{shift.detected_shift.replacement_term}
									</span>
								</div>
								<div className="regime-meta">
									<Badge
										level={shift.synchronization_label}
										label={`${shift.synchronization_label} sync`}
									/>
									<span>
										{shift.observed_across} of {shift.total_sources} sources
									</span>
									<span>
										synchronization score:{" "}
										{shift.synchronization_score.toFixed(2)}
									</span>
								</div>
